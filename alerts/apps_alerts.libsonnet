    ...
    groups+: [
      {
        ...
        rules_:: [
          ...
          {
            expr: |||
              sum by (namespace, pod, container, job, reason, %(clusterLabel)s) (kube_pod_container_status_waiting_reason{reason!="CrashLoopBackOff", %(prefixedNamespaceSelector)s%(kubeStateMetricsSelector)s}) > 0
            ||| % $._config,
            labels: {
              severity: 'warning',
            },

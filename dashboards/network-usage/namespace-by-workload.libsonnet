    ...
    'namespace-by-workload.json':
      ...
      local typeTemplate =
        template.new(
          ...
          auto_count: 30,
          auto_min: '10s',
          definition: 'label_values(mixin_pod_workload{%(clusterLabel)s="$cluster",%(clusterLabel)s="$cluster",namespace=~"$namespace", workload=~".+"}, workload_type)' % $._config,
          skipUrlSync: false,
        };
      ...
      .addPanel(
        newBarplotPanel(
          ...
          graphQuery=|||
            ...
            * on (namespace,pod)
            group_left(workload,workload_type) mixin_pod_workload{%(clusterLabel)s="$cluster",namespace=~"$namespace", workload=~".+", workload_type="$type"}) by (workload))
          ||| % $._config,
          legendFormat='{{ workload }}',
        ),

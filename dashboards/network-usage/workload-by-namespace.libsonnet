...

{
  grafanaDashboards+:: {
    ...
    'workload-by-namespace.json':
      ...
      dashboard.new(
        title='%(dashboardNamePrefix)sNetworking / Workload (Namespace)' % $._config.grafanaK8s,
      ...
      .addPanel(
        newPieChartPanel(
          ...
          pieChartQuery=|||
            ...
            * on (namespace,pod)
            group_left(workload,workload_type) mixin_pod_workload{namespace=~".+", workload=~"$workload", workload_type="$type"}) by (namespace))
          |||,
        ),

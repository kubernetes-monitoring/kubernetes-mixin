    ...
    'k8s-cluster-rsrc-use.json':
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.queryPanel(|||
            sum(max(node_filesystem_size{fstype=~"ext[24]|xfs"} - node_filesystem_avail{fstype=~"ext[24]|xfs"}) by (device,%(podLabel)s,namespace)) by (%(podLabel)s,namespace) / scalar(sum(max(node_filesystem_size{fstype=~"ext[24]|xfs"}) by (device,%(podLabel)s,namespace))) * on (namespace, %(podLabel)s) group_left(node) node_namespace_pod:kube_pod_info:
    ...
    'k8s-node-rsrc-use.json':
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.queryPanel('1 - sum(max by (device, node) (node_filesystem_avail{fstype=~"ext[24]|xfs"})) / sum(max by (device, node) (node_filesystem_size{fstype=~"ext[24]|xfs"}))', 'Disk') +
          { yaxes: g.yaxes('percentunit') },
        ),
      ),
  },
